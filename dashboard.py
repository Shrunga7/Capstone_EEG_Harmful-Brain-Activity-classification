"""
HMS EEG Real-Time Classification Dashboard
-------------------------------------------
Single file. No extra dependencies beyond what your notebooks already use.

Requirements:
    pip install fastapi uvicorn websockets torch timm numpy pandas pyarrow opencv-python

Folder layout expected:
    ./best_transformer_model.pth        ← saved by your LSTM notebook
    ./train_spectrograms/               ← folder of <spectrogram_id>.parquet files
...
# ─────────────────────────────────────────────────────────────────────────────
MODEL_PATH      = "best_transformer_model.pth"   # output from your LSTM notebook
...
# ─────────────────────────────────────────────────────────────────────────────
# MODEL — exact copy of HybridLSTMSpectrogramModel from CapstoneProjectLSTM.ipynb
# ─────────────────────────────────────────────────────────────────────────────
class HybridLSTMSpectrogramModel(nn.Module):
    def __init__(self, num_classes=6, hidden_size=256, num_layers=2):
        super().__init__()
        # EfficientNet-B0 backbone, deepest block → 320 channels
        self.cnn = timm.create_model(
            "efficientnet_b0",
            pretrained=False,       # weights come from .pth, not ImageNet
            in_chans=1,
            features_only=True,
            out_indices=[4]
        )
    ...
    def forward(self, x):
        ...
        features = features.mean(dim=2)       # pool frequency → (B, 320, W)
        features = features.permute(0, 2, 1)  # time-first    → (B, W, 320)
        lstm_out, _ = self.lstm(features)     # (B, W, 512)
        x = lstm_out.mean(dim=1)              # global avg pool → (B, 512)
...
# ─────────────────────────────────────────────────────────────────────────────
# PREPROCESSING — mirrors HMSDataset.__getitem__ from CapstoneProjectLSTM.ipynb
# ─────────────────────────────────────────────────────────────────────────────
def preprocess_parquet(path: str):
    """
    1. Load parquet, drop 'time' column if present
    2. NaN → 0 then log1p  (same as notebook)
    3. Centre-crop or pad time axis to 256 steps
    4. cv2.resize frequency axis to 128
    5. Standardise (mean/std)
    6. Transpose → (128, 256), add batch+channel dims → (1, 1, 128, 256)
    """
    try:
        ...
        spec = cv2.resize(spec, (128, 256))           # (256 rows, 128 cols)
        spec = (spec - spec.mean()) / (spec.std() + 1e-6)
        spec = spec.T                                  # (128, 256) — Freq × Time

        tensor = torch.tensor(spec, dtype=torch.float32).unsqueeze(0)  # (1, 128, 256)
        return tensor.unsqueeze(0).to(DEVICE)                           # (1, 1, 128, 256)
    except Exception as e:
        print(f"  Preprocess error [{os.path.basename(path)}]: {e}")
        return None


# ─────────────────────────────────────────────────────────────────────────────
# STREAMER — walks through parquet files one by one, yields live predictions
# ─────────────────────────────────────────────────────────────────────────────
class SpectrogramStreamer:
    def __init__(self):
        ...
        if self.total == 0:
            print(f"⚠️  No .parquet files found in '{SPECTROGRAM_DIR}/'")
        else:
            print(f"📂 {self.total} spectrogram files ready to stream")

    # ── public method called every STREAM_INTERVAL seconds ──
    def next(self) -> dict:
...
#app{position:relative;z-index:1;display:grid;
     grid-template-rows:50px 1fr 34px;height:100vh;}

/* HEADER */
header{display:flex;align-items:center;justify-content:space-between;
  padding:0 24px;border-bottom:1px solid var(--bdr2);
  background:rgba(11,17,24,.92);backdrop-filter:blur(8px);}
.brand{display:flex;align-items:center;gap:10px;}
.pulse{width:10px;height:10px;border-radius:50%;background:var(--sz);
  box-shadow:0 0 0 3px rgba(255,45,85,.15);animation:beat 1.8s ease-in-out infinite;}
...
.brand-sub{font-size:9px;letter-spacing:.16em;color:var(--mut);text-transform:uppercase;}
.hpills{display:flex;align-items:center;gap:16px;}
...
.hmeta{display:flex;gap:22px;}
.hm{display:flex;flex-direction:column;align-items:flex-end;}
.hml{font-size:8px;letter-spacing:.16em;color:var(--mut);text-transform:uppercase;}
.hmv{font-size:12px;font-weight:500;letter-spacing:.06em;}
...
.sh{display:flex;align-items:center;gap:8px;
  padding:11px 18px 9px;border-bottom:1px solid var(--bdr);flex-shrink:0;}
.sh-line{flex:1;height:1px;background:var(--bdr);}
.sl{font-size:8px;letter-spacing:.22em;text-transform:uppercase;
  color:var(--mut);white-space:nowrap;}

/* STATE CARD */
.state-card{margin:14px 16px 0;border:1px solid var(--bdr2);border-radius:3px;
...
.state-name{font-family:'Syne',sans-serif;font-weight:800;font-size:30px;
  color:var(--ca,var(--oth));transition:color .5s;line-height:1;letter-spacing:.03em;}
.state-row{display:flex;justify-content:space-between;align-items:flex-end;margin-top:8px;}
.state-conf{font-size:11px;color:var(--mut);letter-spacing:.1em;}
.state-conf b{color:var(--txt);font-weight:500;}
.risk{font-size:9px;letter-spacing:.16em;text-transform:uppercase;
...
/* PROB BARS */
.probs{padding:12px 16px;flex-shrink:0;}
.pb-row{display:flex;align-items:center;gap:8px;margin-bottom:7px;}
.pb-name{width:52px;font-size:10px;letter-spacing:.07em;color:var(--mut);
  text-align:right;flex-shrink:0;}
.pb-track{flex:1;height:9px;background:rgba(255,255,255,.04);
  border-radius:1px;overflow:hidden;}
.pb-fill{height:100%;border-radius:1px;transition:width .65s cubic-bezier(.4,0,.2,1);}
.pb-pct{width:38px;font-size:10px;text-align:right;color:var(--txt);flex-shrink:0;}

/* FILE PROGRESS */
.file-info{margin:0 16px 14px;padding:10px 14px;
  border:1px solid var(--bdr2);border-radius:3px;flex-shrink:0;}
.fi-row{display:flex;justify-content:space-between;
  font-size:10px;color:var(--mut);letter-spacing:.08em;margin-bottom:6px;}
.fi-row b{color:var(--txt);}
.prog-track{height:4px;background:rgba(255,255,255,.05);border-radius:2px;overflow:hidden;}
.prog-fill{height:100%;border-radius:2px;transition:width .6s ease,background .5s;}

/* RIGHT COL */
.eeg-wrap{flex-shrink:0;padding:10px 16px 10px;height:180px;}
...
.sb{background:var(--sur);padding:9px 14px;}
.sv{font-family:'Syne',sans-serif;font-weight:700;font-size:17px;color:#fff;}
.sk{font-size:8px;letter-spacing:.16em;text-transform:uppercase;color:var(--mut);margin-top:1px;}

/* LOG */
.log-hdr{display:grid;grid-template-columns:72px 90px 1fr 52px 60px;
  padding:7px 18px;border-bottom:1px solid var(--bdr2);flex-shrink:0;
  font-size:8px;letter-spacing:.16em;text-transform:uppercase;color:var(--mut);}
.log-wrap{flex:1;overflow-y:auto;min-height:0;
  scrollbar-width:thin;scrollbar-color:var(--bdr2) transparent;}
.log-row{display:grid;grid-template-columns:72px 90px 1fr 52px 60px;
...
.lr-time{font-size:10px;color:var(--mut);}
.lr-id{font-size:10px;color:var(--mut);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
.cls-tag{font-size:9px;letter-spacing:.12em;font-weight:600;text-transform:uppercase;
  padding:2px 7px;border-radius:2px;}
.lr-conf{font-size:10px;text-align:right;color:var(--txt);}
.lr-bar-wrap{padding:0 4px;}
.lr-bar{height:5px;border-radius:2px;}

/* FOOTER */
footer{display:flex;align-items:center;justify-content:space-between;
  padding:0 24px;border-top:1px solid var(--bdr);background:var(--sur);
  font-size:9px;letter-spacing:.13em;text-transform:uppercase;color:var(--mut);}
#cdot{width:5px;height:5px;border-radius:50%;background:var(--mut);
...
<main>
  <!-- LEFT: prediction + probs + file progress -->
  <div class="col">
  ...
  <div class="col">
    ...
    <div class="stats">
      ...
      <div class="sb"><div class="sv" id="stCls" style="font-size:13px">—</div><div class="sk">Last Class</div></div>
    ...
    <div class="log-hdr">
      ...
      <span style="text-align:right">Conf</span><span></span>
    </div>
...
function drawEEG(){
  ...
  for(let c=0;c<NCH;c++){
    ...
    ctx.beginPath();ctx.strokeStyle=col;ctx.lineWidth=.85;ctx.globalAlpha=.72;
    for(let x=0;x<w;x++){
      const t=(x/w)+eegT;
      let y=Math.sin(t*CF[c]*Math.PI*2+CPH[c])*amp;
      y+=Math.sin(t*CF[c]*1.6*Math.PI*2+CPH[c]*.5)*amp*.2;
      y+=(Math.random()-.5)*amp*.08;
      // modulate waveform shape by predicted class
    ...
    ctx.fillStyle='#2a4860';ctx.font='8px "IBM Plex Mono"';
    ctx.fillText(CNAMES[c],3,c*rH+9);
  }
}
...
// ── Update all panels from one WebSocket message ──
function update(d){
  curCls=d.predicted;
